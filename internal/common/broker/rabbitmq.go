...
func HandleRetry(ctx context.Context, ch *amqp.Channel, d *amqp.Delivery) error {
	logrus.Info("max-retry-count", maxRetryCount)
	if d.Headers == nil {
		d.Headers = amqp.Table{}
	}
	retryCount, ok := d.Headers[amqpRetryHeaderKey].(int64)
	if !ok {
		logrus.Warnf("Error Get Headers amqpRetryHeaderKey")
		retryCount = 0
	}
	retryCount++
	...
	logrus.Infof("retring messages %s count = %d", d.MessageId, retryCount)
